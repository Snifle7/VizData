 Shader "Custom/07HorizontalRepeatedGradient"
{
	...
	SubShader
	{
		Pass
		{
			...
			{
				int gradientCount = 3;
				half brightness = frac(i.uv.x * gradientCount);
				//half brightness = fmod(i.uv.x * gradientCount,1); //does same as above
				return half4(brightness, brightness,brightness, 1);
				//return half4(brightness.xxx,1)//does same as  above
			}
